    ...
    public partial class HotelReservationApp : Form
    {
        ...
        private void InitializeFiles()
        {
            ...
            foreach(Hotel hotel in elTuristiko.hotels)
            {
                ...
                {
                    textBox1.Text += (r.No + "\t" + r.Price + "\t" + r.GetType()).ToString() + "\t" + r.HasAC +"\t" + r.HasBalcony + "\t" + r.HasMinibar +  "\t" + r.HasSeaView + "\t" + r.HasTV + "\r\n";
                }
            }
        ...
        private void AddRoomsRandom(Hotel hotel, int n)
        {
            ...
            for (int i = 1, j = 100; i <= n; i++)
            {
                ...
                if(rnd_number < 0.15)
                {
                    hotel.AddRoom(new SingleRoom(j + i % 10, random.Next(60,150), random.NextDouble() >= 0.25, random.NextDouble() >= 0.35, random.NextDouble() >= 0.5, random.NextDouble() >= 0.3, random.NextDouble() >= 0.3));
                }else if(rnd_number < 0.35)
                {
                    hotel.AddRoom(new DoubleRoom(j + i % 10, random.Next(100, 240), random.NextDouble() >= 0.25, random.NextDouble() >= 0.35, random.NextDouble() >= 0.5, random.NextDouble() >= 0.3, random.NextDouble() >= 0.3));
                }else if(rnd_number < 0.70)
                {
                    hotel.AddRoom(new TwinRoom(j + i % 10, random.Next(120, 280), random.NextDouble() >= 0.25, random.NextDouble() >= 0.35, random.NextDouble() >= 0.5, random.NextDouble() >= 0.3, random.NextDouble() >= 0.3));
                }else if(rnd_number < 0.90)
                {
                    hotel.AddRoom(new TripleRoom(j + i % 10, random.Next(140, 360), random.NextDouble() >= 0.25, random.NextDouble() >= 0.35, random.NextDouble() >= 0.5, random.NextDouble() >= 0.3, random.NextDouble() >= 0.3));
                }
                else
                {
